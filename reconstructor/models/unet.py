...
class UNet(nn.Module):
    ...
    def forward(
        ...
        """
        This UNet module may be used as either a regularization module or as
        the main reconstruction network.
        Input:
            image: estimated image reconstruction of shape BCHW.
        Returns:
            Output regularization tensor of shape BCHW (C is out_chans) or
...
class NormUNet(nn.Module):
    ...
    def norm(
        ...
        Input:
            x: input tensor of shape B(2C)HW.
        Returns:
            norm_x: output tensor of shape B(2C)HW.
        ...
        if c2 % 2 != 0:
            raise RuntimeError(f"Unexpected dimensions {x.size()}")
        x = x.view(b, 2, c2 * h * w // 2)

        mean = torch.mean(x, dim=2, keepdim=True)
        std = torch.std(x, dim=2, keepdim=True)

        xnorm = (x - mean) / std
        xnorm = xnorm.view(b, c2, h, w)
    ...
    def unnorm(
        ...
        Input:
            x: normalized input tensor of shape B(2C)HW.
            mean: mean of both real and imaginary components of each batch.
            std: STD of both real and imaginary componetns of each batch.
        Returns:
            Unnormalized output tensor of shape B(2C)HW.
        """
        b, c2, h, w = x.size()
        if c2 % 2 != 0:
            raise RuntimeError(f"Unexpected dimensions {x.size()}")
        x = x.reshape(b, 2, c2 * h * w // 2)
        x = (x * std) + mean

        return x.view(b, c2, h, w)
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        """
        Input:
            image: input tensor of shape NCHW (C is in_chans).
        Returns:
        ...
        image, h_pad, w_pad, h_ceil, w_ceil = self.pad(image)

        # Feed into UNet.
        image = self.unet(image)

        # Unnormalize and restore original shape.
        image = self.unpad(image, h_pad, w_pad, h_ceil, w_ceil)
        image = self.unnorm(image, mean, std)
        image = self.chan_complex_to_last_dim(image)
